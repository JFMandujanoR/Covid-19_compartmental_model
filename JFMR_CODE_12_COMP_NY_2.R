...
covid = covid[which(covid$Province_State == "New York"),]

covid = covid[which(covid$Admin2 %in% c("New York","Bronx","Queens","Kings","Richmond")),]
...
muertos = muertos[which(muertos$Province_State == "New York"),]

muertos = muertos[which(muertos$Admin2 %in% c("New York","Bronx","Queens","Kings","Richmond")),]
...
ggplot(dat,aes(x=time))+
  ...
  labs(y="Population", x = "Time in days after 03-05-2020") + theme_classic()
...
ggplot(x,aes(x=time))+
  ...
  labs(y="Population", x = "Time in days after 03-05-2020")
...
ggplot(ci_compart[ci_compart$compartment != "S" & ci_compart$compartment != "R",], aes(x=time))+
  ...
  theme(legend.title=element_text(size=12,face="bold"),
        ...
        legend.key=element_rect(colour="#FFFFFF",
                                ...
                                linetype="solid"))+
  guides(colour = guide_legend(override.aes = list(size=2)))+ ggtitle("Stochastic compartments") +
  labs(y="Population", x = "Time in days after 03-05-2020")  + theme_classic() + ylim(0, 30000)
