...
covid = covid[which(covid$Province_State == "Wisconsin"),]
covid = covid[which(covid$Admin2 == "Dane"),]
covid = covid[names(covid)[ 13: length(names(covid)) ]]
covid = as.data.frame(colSums(covid))
#covid_19 = as.data.frame(tail(covid,-46))
#covid_19 = as.data.frame(tail(covid,-46))
#rownames(covid)
covid_19 = as.data.frame(tail(covid,-52))
#covid['X3.15.20',]
...
muertos = muertos[which(muertos$Province_State == "Wisconsin"),]
muertos = muertos[which(muertos$Admin2 == "Dane"),]
...
muertos_19 = as.data.frame(tail(muertos,-53))
...
pomp(
  data=subset(act_rec), #, select = -date
  times="time",t0=0,
  ...
  rinit=Csnippet("
      ...
      Irs = Irs_0;
      Irh = Irh_0;
      C = Itp_0+Is_0+Ih_0+Iv_0+Irs_0+Irh_0+Etp_0;
      R = R_0;
      D = D_0;"),
  statenames=c("S","E0","Etp","Itn","Itp","Is","Ih","Iv","Irs","Irh","R","D","C"),
  paramnames=
...
sse <- function (params) {
  ...
  discrep2 = ( (x["C"])-obs(deter.model) )^2
  ...
  #return(  sqrt(sum(discrep2))/( length(obs(deter.model) )  ) ) 
  return( sum(discrep2) ) 
}
...
fit2 = optim(fn=f2,par=c(0.20, 0.291, 0.82, 0.183, 0.50, 0.03, 0.25, 0.50, 0.50, 0.65, 
             ...
             lower=c(0.07, 1/4, 0.7, 1/6, 0.10, 0.02, 0.10, 0.10, 0.10,  0.40, 
                     1/14, 1/14, 0, 0, 0, 1),
             upper=c(0.90, 1/3, 0.95, 1/5, 0.90, 0.04, 0.40, 0.90, 0.90, 0.90,
                     1/3, 1/3, 500, 1, 1, 10), 
             method="L-BFGS-B")
...
ggplot(dat,aes(x=time))+
  ...
  geom_line(aes(y=C,color='C'), size=2) + ggtitle("Computed Cumulative v.s. Cumulative cases") +
  labs(y="Population", x = "Time in days after 03-15-2020")

##### daily cases
ggplot(dat,aes(x=time))+
  geom_line(aes(y=c(diff(data),0),color='data'), size=2)+
  geom_line(aes(y=c(diff(C),0),color='N'), size=2) + ggtitle("Computed Daily v.s. Daily cases") +
  labs(y="Population", x = "Time in days after 03-15-2020")
...
pomp(data=data.frame(time=1:350,data=NA),
     times="time",t0=0,
...
R = max(abs(eigen(mat_F%*%solve(mat_V))$val))
...
sir_step <- Csnippet("
      ...
      C += dN_E0Itn+dN_EtpItp+dN_ItpIs+dN_IsIh+dN_IhIv+dN_IsIrs+dN_IhIrh+dN_IvIrh+dN_E0Etp;
...
init <- Csnippet("
      ...
      Irs = Irs_0;
      Irh = Irh_0;
      C = Itn_0+Itp_0+Is_0+Ih_0+Iv_0+Irs_0+Irh_0+Etp_0;
      R = R_0;
      D = D_0;")

tiempos = 350

rmeas <- Csnippet("covid = C;")
